...
quantile(d$gdppc, seq(0, 1, 0.1), na.rm = TRUE) # Quintile # Can be extremely helpful when researching social media data with big KOLs
```
...
Problem: Extremely skewed - because there are many countries with low GDP per capita

But...if I want to know the distribution in the range with highest density?

## Skewed Distribution

-   **Challenge:** Quantitative data may have skewed distributions, which can make the visualization uninformative and lack of details.
...
d |> ggplot(aes(x = gdppc)) + geom_histogram() + 
  ...
  #hjust = 0 -> put text at the right of the line
  labs(x = "GDP per capita", y = "Count", title = "Histogram of GDP per capita")
# How do I get y = 7500? Observe the y-axis in the previous figure manually.
```
...
d |> ggplot(aes(x = gdppc)) + geom_histogram() + 
  ...
  # x = gdppc, label = anno, etc.: More automated!
  labs(x = "GDP per capita", y = "Count", title = "Histogram of GDP per capita")
```

\normalsize

Why bother with the automated approach? It helps when you want to annotate **multiple** cases and reduce the possibility of mistakes.
...
d |> ggplot(aes(x = gdppc, y = life_expectancy)) +
  ...
  # It does not make sense in this case because the distribution is NOT linear
  labs(x = "GDP per capita", y = "Life Expectancy", title = "Wealth and Health in the World (1800-2019)")
```
